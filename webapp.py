...
def render_first2():
    ...
    if 'counties' in request.args:
        return render_template('page2.html', states = get_state_options(counties), average_issued = average_awarded(get_county_state(request.args['counties'],counties), counties), counties = get_county_options(get_county_state(request.args['counties'],counties),counties), county_awarded = get_county_awarded(request.args['counties'],counties))
    if 'states' in request.args:
        return render_template('page2.html', states = get_state_options(counties), average_issued = average_awarded(request.args['states'], counties), counties = get_county_options(request.args['states'],counties))
